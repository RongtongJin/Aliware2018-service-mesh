...
/**
 * 固定长度的CompositeByteBuf，针对其中一个长度可变，其他长度或内容不可变
 */
public class FixedLengthCompositeByteBuf  extends AbstractReferenceCountedByteBuf implements Iterable<ByteBuf>  {
    ...
    /**
     * 考虑到减少操作，需要传递等长度的ByteBuf，长度可变的ByteBuf传递EmptyByteBuf实例就行。长度或内容不变的传递等长度或者同内容的ByteBuf即可
     * @see EmptyByteBuf
     * @param alloc
     * @param buffers
     */
    public FixedLengthCompositeByteBuf(ByteBufAllocator alloc, ByteBuf ...buffers) {
        super(Integer.MAX_VALUE);
        this.alloc = alloc;
        this.direct = true;
        this.maxNumComponents = buffers.length;
        components = newList(maxNumComponents);
        this.addComponents(buffers);
    }
     ...
     * @return
     */
    public FixedLengthCompositeByteBuf setLastComponent(ByteBuf buffer) {
        setComponent(components.size() - 1, buffer, true);
        return this;
    }
     ...
     * @return
     */
    public FixedLengthCompositeByteBuf setComponent(int index, ByteBuf buffer) {
        setComponent(index, buffer, true);
        return this;
    }
     ...
     * @param index
     * @param buffer
     * @param updateComponentIndex 当buffer内容改变，长度不变时，不需要更新索引
     * @return
     */
    public FixedLengthCompositeByteBuf setComponent(int index, ByteBuf buffer, boolean updateComponentIndex) {
        Component comp = components.get(index);
        comp.buf = buffer.slice();
        if (updateComponentIndex) {
            int length = buffer.readableBytes();
            comp.length = length;
            comp.endOffset = comp.offset + length;

            for (int ind = index + 1; ind < components.size(); ++ind) {
                Component nextComp = components.get(ind);
                nextComp.offset = comp.endOffset;
                nextComp.endOffset = nextComp.offset + nextComp.length;

                comp = nextComp;
            }
            // 更新
            writerIndex(comp.endOffset);
        }
        return this;
    }
     ...
     * <p>
     * Be aware that this method does not increase the {@code writerIndex} of the {@link FixedLengthCompositeByteBuf}.
     * If you need to have it increased use {@link #addComponent(boolean, ByteBuf)}.
     * <p>
     * {@link ByteBuf#release()} ownership of {@code buffer} is transfered to this {@link FixedLengthCompositeByteBuf}.
     * @param buffer the {@link ByteBuf} to add. {@link ByteBuf#release()} ownership is transfered to this
     * {@link FixedLengthCompositeByteBuf}.
     */
    public FixedLengthCompositeByteBuf addComponent(ByteBuf buffer) {
     ...
     * ownership of all {@link ByteBuf} objects is transfered to this {@link FixedLengthCompositeByteBuf}.
     */
    public FixedLengthCompositeByteBuf addComponents(ByteBuf... buffers) {
     ...
     * ownership of all {@link ByteBuf} objects is transfered to this {@link FixedLengthCompositeByteBuf}.
     */
    public FixedLengthCompositeByteBuf addComponents(Iterable<ByteBuf> buffers) {
        return addComponents(false, buffers);
    }
     ...
     * {@link FixedLengthCompositeByteBuf}.
     */
    public FixedLengthCompositeByteBuf addComponent(int cIndex, ByteBuf buffer) {
     ...
     * {@link FixedLengthCompositeByteBuf}.
     */
    public FixedLengthCompositeByteBuf addComponent(boolean increaseWriterIndex, ByteBuf buffer) {
     ...
     * ownership of all {@link ByteBuf} objects is transfered to this {@link FixedLengthCompositeByteBuf}.
     */
    public FixedLengthCompositeByteBuf addComponents(boolean increaseWriterIndex, ByteBuf... buffers) {
     ...
     * ownership of all {@link ByteBuf} objects is transfered to this {@link FixedLengthCompositeByteBuf}.
     */
    public FixedLengthCompositeByteBuf addComponents(boolean increaseWriterIndex, Iterable<ByteBuf> buffers) {
     ...
     * {@link FixedLengthCompositeByteBuf}.
     */
    public FixedLengthCompositeByteBuf addComponent(boolean increaseWriterIndex, int cIndex, ByteBuf buffer) {
        checkNotNull(buffer, "buffer");
     ...
     * ownership of all {@link ByteBuf} objects is transfered to this {@link FixedLengthCompositeByteBuf}.
     */
    public FixedLengthCompositeByteBuf addComponents(int cIndex, ByteBuf... buffers) {
     ...
     * {@link FixedLengthCompositeByteBuf}.
     */
    public FixedLengthCompositeByteBuf addComponents(int cIndex, Iterable<ByteBuf> buffers) {
    ...
    private int addComponents0(boolean increaseIndex, int cIndex, Iterable<ByteBuf> buffers) {
        if (buffers instanceof ByteBuf) {
            // If buffers also implements ByteBuf (e.g. FixedLengthCompositeByteBuf), it has to go to addComponent(ByteBuf).
     ...
     */
    public FixedLengthCompositeByteBuf removeComponent(int cIndex) {
        checkComponentIndex(cIndex);
     ...
     */
    public FixedLengthCompositeByteBuf removeComponents(int cIndex, int numComponents) {
    ...
    @Override
    public FixedLengthCompositeByteBuf capacity(int newCapacity) {
    ...
    @Override
    public FixedLengthCompositeByteBuf getBytes(int index, byte[] dst, int dstIndex, int length) {
    ...
    @Override
    public FixedLengthCompositeByteBuf getBytes(int index, ByteBuffer dst) {
        int limit = dst.limit();
    ...
    @Override
    public FixedLengthCompositeByteBuf getBytes(int index, ByteBuf dst, int dstIndex, int length) {
    ...
    @Override
    public FixedLengthCompositeByteBuf getBytes(int index, OutputStream out, int length) throws IOException {
        checkIndex(index, length);
        if (length == 0) {
            return this;
        }
    ...
    @Override
    public FixedLengthCompositeByteBuf setByte(int index, int value) {
        Component c = findComponent(index);
    ...
    @Override
    public FixedLengthCompositeByteBuf setShort(int index, int value) {
        return (FixedLengthCompositeByteBuf) super.setShort(index, value);
    }
    ...
    @Override
    public FixedLengthCompositeByteBuf setMedium(int index, int value) {
        return (FixedLengthCompositeByteBuf) super.setMedium(index, value);
    }
    ...
    @Override
    public FixedLengthCompositeByteBuf setInt(int index, int value) {
        return (FixedLengthCompositeByteBuf) super.setInt(index, value);
    }
    ...
    @Override
    public FixedLengthCompositeByteBuf setLong(int index, long value) {
        return (FixedLengthCompositeByteBuf) super.setLong(index, value);
    }
    ...
    @Override
    public FixedLengthCompositeByteBuf setBytes(int index, byte[] src, int srcIndex, int length) {
    ...
    @Override
    public FixedLengthCompositeByteBuf setBytes(int index, ByteBuffer src) {
        int limit = src.limit();
    ...
    @Override
    public FixedLengthCompositeByteBuf setBytes(int index, ByteBuf src, int srcIndex, int length) {
     ...
     */
    public FixedLengthCompositeByteBuf consolidate() {
        ensureAccessible();
     ...
     */
    public FixedLengthCompositeByteBuf consolidate(int cIndex, int numComponents) {
        checkComponentIndex(cIndex, numComponents);
        if (numComponents <= 1) {
            return this;
        }
     ...
     */
    public FixedLengthCompositeByteBuf discardReadComponents() {
        ensureAccessible();
    ...
    @Override
    public FixedLengthCompositeByteBuf discardReadBytes() {
        ensureAccessible();
        final int readerIndex = readerIndex();
        if (readerIndex == 0) {
            return this;
    ...
    @Override
    public FixedLengthCompositeByteBuf readerIndex(int readerIndex) {
        return (FixedLengthCompositeByteBuf) super.readerIndex(readerIndex);
    }

    @Override
    public FixedLengthCompositeByteBuf writerIndex(int writerIndex) {
        return (FixedLengthCompositeByteBuf) super.writerIndex(writerIndex);
    }

    @Override
    public FixedLengthCompositeByteBuf setIndex(int readerIndex, int writerIndex) {
        return (FixedLengthCompositeByteBuf) super.setIndex(readerIndex, writerIndex);
    }

    @Override
    public FixedLengthCompositeByteBuf clear() {
        return (FixedLengthCompositeByteBuf) super.clear();
    }

    @Override
    public FixedLengthCompositeByteBuf markReaderIndex() {
        return (FixedLengthCompositeByteBuf) super.markReaderIndex();
    }

    @Override
    public FixedLengthCompositeByteBuf resetReaderIndex() {
        return (FixedLengthCompositeByteBuf) super.resetReaderIndex();
    }

    @Override
    public FixedLengthCompositeByteBuf markWriterIndex() {
        return (FixedLengthCompositeByteBuf) super.markWriterIndex();
    }

    @Override
    public FixedLengthCompositeByteBuf resetWriterIndex() {
        return (FixedLengthCompositeByteBuf) super.resetWriterIndex();
    }

    @Override
    public FixedLengthCompositeByteBuf ensureWritable(int minWritableBytes) {
        return (FixedLengthCompositeByteBuf) super.ensureWritable(minWritableBytes);
    }

    @Override
    public FixedLengthCompositeByteBuf getBytes(int index, ByteBuf dst) {
        return (FixedLengthCompositeByteBuf) super.getBytes(index, dst);
    }

    @Override
    public FixedLengthCompositeByteBuf getBytes(int index, ByteBuf dst, int length) {
        return (FixedLengthCompositeByteBuf) super.getBytes(index, dst, length);
    }

    @Override
    public FixedLengthCompositeByteBuf getBytes(int index, byte[] dst) {
        return (FixedLengthCompositeByteBuf) super.getBytes(index, dst);
    }

    @Override
    public FixedLengthCompositeByteBuf setBoolean(int index, boolean value) {
        return (FixedLengthCompositeByteBuf) super.setBoolean(index, value);
    }

    @Override
    public FixedLengthCompositeByteBuf setChar(int index, int value) {
        return (FixedLengthCompositeByteBuf) super.setChar(index, value);
    }

    @Override
    public FixedLengthCompositeByteBuf setFloat(int index, float value) {
        return (FixedLengthCompositeByteBuf) super.setFloat(index, value);
    }

    @Override
    public FixedLengthCompositeByteBuf setDouble(int index, double value) {
        return (FixedLengthCompositeByteBuf) super.setDouble(index, value);
    }

    @Override
    public FixedLengthCompositeByteBuf setBytes(int index, ByteBuf src) {
        return (FixedLengthCompositeByteBuf) super.setBytes(index, src);
    }

    @Override
    public FixedLengthCompositeByteBuf setBytes(int index, ByteBuf src, int length) {
        return (FixedLengthCompositeByteBuf) super.setBytes(index, src, length);
    }

    @Override
    public FixedLengthCompositeByteBuf setBytes(int index, byte[] src) {
        return (FixedLengthCompositeByteBuf) super.setBytes(index, src);
    }

    @Override
    public FixedLengthCompositeByteBuf setZero(int index, int length) {
        return (FixedLengthCompositeByteBuf) super.setZero(index, length);
    }

    @Override
    public FixedLengthCompositeByteBuf readBytes(ByteBuf dst) {
        return (FixedLengthCompositeByteBuf) super.readBytes(dst);
    }

    @Override
    public FixedLengthCompositeByteBuf readBytes(ByteBuf dst, int length) {
        return (FixedLengthCompositeByteBuf) super.readBytes(dst, length);
    }

    @Override
    public FixedLengthCompositeByteBuf readBytes(ByteBuf dst, int dstIndex, int length) {
        return (FixedLengthCompositeByteBuf) super.readBytes(dst, dstIndex, length);
    }

    @Override
    public FixedLengthCompositeByteBuf readBytes(byte[] dst) {
        return (FixedLengthCompositeByteBuf) super.readBytes(dst);
    }

    @Override
    public FixedLengthCompositeByteBuf readBytes(byte[] dst, int dstIndex, int length) {
        return (FixedLengthCompositeByteBuf) super.readBytes(dst, dstIndex, length);
    }

    @Override
    public FixedLengthCompositeByteBuf readBytes(ByteBuffer dst) {
        return (FixedLengthCompositeByteBuf) super.readBytes(dst);
    }

    @Override
    public FixedLengthCompositeByteBuf readBytes(OutputStream out, int length) throws IOException {
        return (FixedLengthCompositeByteBuf) super.readBytes(out, length);
    }

    @Override
    public FixedLengthCompositeByteBuf skipBytes(int length) {
        return (FixedLengthCompositeByteBuf) super.skipBytes(length);
    }

    @Override
    public FixedLengthCompositeByteBuf writeBoolean(boolean value) {
        return (FixedLengthCompositeByteBuf) super.writeBoolean(value);
    }

    @Override
    public FixedLengthCompositeByteBuf writeByte(int value) {
        return (FixedLengthCompositeByteBuf) super.writeByte(value);
    }

    @Override
    public FixedLengthCompositeByteBuf writeShort(int value) {
        return (FixedLengthCompositeByteBuf) super.writeShort(value);
    }

    @Override
    public FixedLengthCompositeByteBuf writeMedium(int value) {
        return (FixedLengthCompositeByteBuf) super.writeMedium(value);
    }

    @Override
    public FixedLengthCompositeByteBuf writeInt(int value) {
        return (FixedLengthCompositeByteBuf) super.writeInt(value);
    }

    @Override
    public FixedLengthCompositeByteBuf writeLong(long value) {
        return (FixedLengthCompositeByteBuf) super.writeLong(value);
    }

    @Override
    public FixedLengthCompositeByteBuf writeChar(int value) {
        return (FixedLengthCompositeByteBuf) super.writeChar(value);
    }

    @Override
    public FixedLengthCompositeByteBuf writeFloat(float value) {
        return (FixedLengthCompositeByteBuf) super.writeFloat(value);
    }

    @Override
    public FixedLengthCompositeByteBuf writeDouble(double value) {
        return (FixedLengthCompositeByteBuf) super.writeDouble(value);
    }

    @Override
    public FixedLengthCompositeByteBuf writeBytes(ByteBuf src) {
        return (FixedLengthCompositeByteBuf) super.writeBytes(src);
    }

    @Override
    public FixedLengthCompositeByteBuf writeBytes(ByteBuf src, int length) {
        return (FixedLengthCompositeByteBuf) super.writeBytes(src, length);
    }

    @Override
    public FixedLengthCompositeByteBuf writeBytes(ByteBuf src, int srcIndex, int length) {
        return (FixedLengthCompositeByteBuf) super.writeBytes(src, srcIndex, length);
    }

    @Override
    public FixedLengthCompositeByteBuf writeBytes(byte[] src) {
        return (FixedLengthCompositeByteBuf) super.writeBytes(src);
    }

    @Override
    public FixedLengthCompositeByteBuf writeBytes(byte[] src, int srcIndex, int length) {
        return (FixedLengthCompositeByteBuf) super.writeBytes(src, srcIndex, length);
    }

    @Override
    public FixedLengthCompositeByteBuf writeBytes(ByteBuffer src) {
        return (FixedLengthCompositeByteBuf) super.writeBytes(src);
    }

    @Override
    public FixedLengthCompositeByteBuf writeZero(int length) {
        return (FixedLengthCompositeByteBuf) super.writeZero(length);
    }

    @Override
    public FixedLengthCompositeByteBuf retain(int increment) {
        return (FixedLengthCompositeByteBuf) super.retain(increment);
    }

    @Override
    public FixedLengthCompositeByteBuf retain() {
        return (FixedLengthCompositeByteBuf) super.retain();
    }

    @Override
    public FixedLengthCompositeByteBuf touch() {
        return this;
    }

    @Override
    public FixedLengthCompositeByteBuf touch(Object hint) {
        return this;
    }
    ...
    @Override
    public FixedLengthCompositeByteBuf discardSomeReadBytes() {
